...
import { BarChart3, TrendingUp, Scale, Plus, Activity, RefreshCcw } from 'lucide-react-native';
import * as Haptics from 'expo-haptics';
import { useQueryClient } from '@tanstack/react-query';
import { useTheme } from '@/contexts/ThemeContext';
import { useUser } from '@/contexts/UserContext';
import { useLanguage } from '@/contexts/LanguageContext';
import Body3DWebView from '@/components/Body3DWebView';
...
export default function StatsScreen() {
  ...
  const ds = useMemo(() => StyleSheet.create({
    ...
    title: { fontSize: 28, fontFamily: 'Outfit_800ExtraBold', color: colors.text, letterSpacing: -0.5 },
    periodToggle: {
    ...
    periodBtnText: { fontSize: 13, fontFamily: 'Outfit_600SemiBold', color: colors.textSecondary },
    ...
    cardTitle: { fontSize: 16, fontFamily: 'Outfit_700Bold', color: colors.text, marginBottom: 14 },
    summaryValue: { fontSize: 28, fontFamily: 'Outfit_800ExtraBold', color: colors.text, letterSpacing: -1 },
    summaryLabel: { fontSize: 12, color: colors.textSecondary, fontFamily: 'Outfit_500Medium', marginTop: 2 },
    summaryCard: {
      flex: 1,
      backgroundColor: colors.surfaceSecondary,
      borderRadius: 16,
    ...
    streakValue: { fontSize: 44, fontFamily: 'Outfit_800ExtraBold', color: colors.primary, letterSpacing: -2 },
    streakLabel: { fontSize: 14, color: colors.textSecondary, fontFamily: 'Outfit_500Medium' },
    weightBtn: {
      flexDirection: 'row' as const,
      alignItems: 'center' as const,
      gap: 6,
    ...
    weightBtnText: { fontSize: 13, fontFamily: 'Outfit_600SemiBold', color: colors.primary },
    weightInput: {
      backgroundColor: colors.surfaceSecondary,
      borderRadius: 14,
      paddingHorizontal: 16,
    ...
    weightSaveText: { fontSize: 14, fontFamily: 'Outfit_600SemiBold', color: colors.white },
    ...
    healthSyncBtnText: { fontSize: 15, fontFamily: 'Outfit_600SemiBold', color: colors.white },
    healthDataRow: { flexDirection: 'row', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, marginTop: 10 },
    healthDataItem: {
      backgroundColor: colors.surfaceSecondary,
      borderRadius: 14,
      padding: 12,
    ...
    healthDataLabel: { fontSize: 12, fontFamily: 'Outfit_500Medium', color: colors.textSecondary, marginBottom: 4 },
    healthDataValue: { fontSize: 20, fontFamily: 'Outfit_700Bold', color: colors.text },
  }), [colors]);
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        <Animated.View style={{ flex: 1, opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          >
            ...
            <View style={[ds.card, { alignItems: 'center' }]}>
              <Text style={ds.cardTitle}>Tana Vizualizatsiyasi (3D)</Text>
              <Body3DWebView 
                stats={{ 
                  gender: profile.gender || 'male', 
                  age: profile.age || 25, 
                  heightCm: profile.height || 170, 
                  weightKg: recentWeight?.weight || profile.weight || 70 
                }} 
              />
            </View>
            ...
            <View style={ds.card}>
              <View style={styles.cardTitleRow}>
                ...
                <Text style={ds.cardTitle}>Sog'liq Ma'lumotlari (Health)</Text>
              </View>
              {healthData?.isSynced ? (
                <>
                  <View style={ds.healthDataRow}>
                    <View style={ds.healthDataItem}>
                      <Text style={ds.healthDataLabel}>Qadamlar</Text>
                      <Text style={ds.healthDataValue}>{healthData.steps.toLocaleString()}</Text>
                    </View>
                    <View style={ds.healthDataItem}>
                      <Text style={ds.healthDataLabel}>Masofa</Text>
                      <Text style={ds.healthDataValue}>{healthData.distance.toFixed(2)} km</Text>
                    </View>
                    <View style={[ds.healthDataItem, { flexBasis: '100%' }]}>
                      <Text style={ds.healthDataLabel}>Faol Kaloriyalar (Active Energy)</Text>
                      <Text style={ds.healthDataValue}>{Math.round(healthData.activeCalories)} kkal</Text>
                    </View>
                  </View>
                  <TouchableOpacity style={[ds.healthSyncBtn, { backgroundColor: colors.surfaceSecondary }]} onPress={syncHealthData}>
                    <RefreshCcw size={16} color={colors.primary} />
                    <Text style={[ds.healthSyncBtnText, { color: colors.primary }]}>Yangilash</Text>
                  </TouchableOpacity>
                </>
              ) : (
                <TouchableOpacity style={ds.healthSyncBtn} onPress={syncHealthData}>
                  <RefreshCcw size={18} color={colors.white} />
                  <Text style={ds.healthSyncBtnText}>Qurilmani ulash (Sinxronizatsiya)</Text>
                </TouchableOpacity>
              )}
            </View>
...
const styles = StyleSheet.create({
  ...
  macroCircleValue: { fontSize: 18, fontFamily: 'Outfit_800ExtraBold' },
