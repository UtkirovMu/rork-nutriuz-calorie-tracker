...
export default function ChatScreen() {
  ...
  const quickAnims = useRef(Array.from({ length: 4 }, () => new Animated.Value(0))).current;
...
13. Agar foydalanuvchi ma'lumot (makro, ovqat rejasi, va h.k.) so'rasa, chiroyli Markdown jadvallar (Tables) ko'rinishida ber!`;
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.timing(emptyFade, { toValue: 1, duration: 600, delay: 100, useNativeDriver: true }),
      Animated.spring(emptySlide, { toValue: 0, tension: 70, friction: 10, delay: 100, useNativeDriver: true }),
      Animated.timing(inputBarFade, { toValue: 1, duration: 400, delay: 50, useNativeDriver: true }),
      Animated.spring(inputBarSlide, { toValue: 0, tension: 80, friction: 14, useNativeDriver: true }),
    ]).start(() => {
      const stagger = quickAnims.map((anim, i) =>
        Animated.spring(anim, { toValue: 1, tension: 80, friction: 10, delay: i * 100, useNativeDriver: true })
      );
      Animated.parallel(stagger).start();
    });
  }, [headerFade, emptyFade, emptySlide, inputBarFade, inputBarSlide, quickAnims]);
      ...
      >
        {isUser ? (
          <View style={ds.userRow}>
            <View style={ds.userTail} />
            <View style={ds.userTailMask} />
            <LinearGradient colors={isDark ? ['#0A84FF', '#005BB5'] : ['#007AFF', '#0056B3']} style={ds.userBubbleGradient} />
        ...
        >
          ...
          {messages.length === 0 ? (
            ...
            <FlatList
              ...
              ListHeaderComponent={ListFooter} // Footer becomes header because of inverted list
            />
          )}

          <SafeAreaView edges={['bottom']} style={ds.inputBarOuter}>
            {messages.length > 0 && !isLoading && (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={ds.suggestedRow} contentContainerStyle={ds.suggestedContent}>
                {['Kaloriya qancha qoldi?', 'Tushlikka nima yeyishni tavsiya qilasiz?', 'BMR nima degani?', 'Suv ichishim kerakmi?'].map((reply, i) => (
                  <Pressable key={i} style={ds.suggestedChip} onPress={() => handleSend(reply)}>
                    <Text style={ds.suggestedText}>{reply}</Text>
                  </Pressable>
                ))}
              </ScrollView>
            )}
