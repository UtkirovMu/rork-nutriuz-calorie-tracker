...
import { Camera, Plus, Flame, TrendingUp, BarChart3, Sparkles, ScanLine, MessageCircle, Heart, Activity } from 'lucide-react-native';
...
const { profile, dailyTargets, todayMeals, todayTotals, removeMeal, streak, healthData, syncHealthData } = useUser();
...
backgroundColor: colors.surface, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 16,
shadowColor: colors.shadow, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 1, shadowRadius: 8, elevation: 2,
},
streakText: { fontSize: 15, fontWeight: '800' as const, color: colors.text },
ringCard: {
alignItems: 'center' as const, marginBottom: 24,
backgroundColor: colors.surface, borderRadius: 36, paddingVertical: 32, paddingHorizontal: 20,
shadowColor: colors.shadow, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 1, shadowRadius: 30, elevation: 6,
},
quickStatRow: { flexDirection: 'row' as const, width: '100%' as const, marginTop: 24, gap: 12 },
quickStatCard: { flex: 1, backgroundColor: colors.surfaceSecondary, borderRadius: 20, paddingVertical: 16, paddingHorizontal: 12, alignItems: 'center' as const },
quickStatValue: { fontSize: 20, fontWeight: '800' as const, color: colors.text },
quickStatLabel: { fontSize: 12, color: colors.textSecondary, fontWeight: '600' as const, marginTop: 4 },
quickLinksRow: { flexDirection: 'row' as const, flexWrap: 'wrap' as const, gap: 14, marginBottom: 24 },
quickLinkCard: {
width: '47%' as unknown as number, backgroundColor: colors.surface, borderRadius: 24, paddingVertical: 20, paddingHorizontal: 16, gap: 12,
shadowColor: colors.shadow, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 1, shadowRadius: 20, elevation: 4,
},
quickLinkIconWrap: { width: 50, height: 50, borderRadius: 18, alignItems: 'center' as const, justifyContent: 'center' as const, overflow: 'hidden' as const },
quickLinkText: { fontSize: 14, fontWeight: '800' as const, color: colors.text, letterSpacing: -0.3 },
analysisCard: {
backgroundColor: colors.surface, borderRadius: 28, padding: 20, marginBottom: 24,
shadowColor: colors.shadow, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 1, shadowRadius: 20, elevation: 4,
},
...
sectionTitle: { fontSize: 22, fontWeight: '800' as const, color: colors.text, letterSpacing: -0.4 },
macroCard: {
backgroundColor: colors.surface, borderRadius: 28, padding: 20,
shadowColor: colors.shadow, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 1, shadowRadius: 20, elevation: 4,
},
...
<Animated.View style={[dynamicStyles.analysisCard, { opacity: cardAnims[3].opacity, transform: [{ translateY: cardAnims[3].translateY }] }]}>
  <View style={dynamicStyles.analysisHeader}>
    <Activity size={18} color={colors.primary} />
    <Text style={dynamicStyles.analysisTitle}>Faollik (Health)</Text>
  </View>
  <View style={[dynamicStyles.quickStatRow, { marginTop: 0 }]}>
    <View style={dynamicStyles.quickStatCard}>
      <Text style={styles.targetEmoji}>🚶‍♂️</Text>
      <Text style={dynamicStyles.quickStatValue}>{healthData?.steps.toLocaleString()}</Text>
      <Text style={dynamicStyles.quickStatLabel}>Qadamlar</Text>
    </View>
    <View style={dynamicStyles.quickStatCard}>
      <Text style={styles.targetEmoji}>📏</Text>
      <Text style={dynamicStyles.quickStatValue}>{healthData?.distance.toFixed(2)}</Text>
      <Text style={dynamicStyles.quickStatLabel}>Masofa (km)</Text>
    </View>
    <View style={dynamicStyles.quickStatCard}>
      <Text style={styles.targetEmoji}>🔥</Text>
      <Text style={dynamicStyles.quickStatValue}>{Math.round(healthData?.activeCalories || 0)}</Text>
      <Text style={dynamicStyles.quickStatLabel}>Faol kkal</Text>
    </View>
  </View>
  <TouchableOpacity
    style={[dynamicStyles.addMealButton, { marginTop: 16, justifyContent: 'center', backgroundColor: colors.primaryLight }]}
    onPress={() => {
      void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
      syncHealthData();
    }}
  >
    ...
    <Text style={dynamicStyles.addMealText}>{healthData?.isSynced ? 'Yangilash' : 'Ma\'lumotlarni Ulash (Sync)'}</Text>
  </TouchableOpacity>
</Animated.View>

<Animated.View style={[styles.macroSection, { opacity: cardAnims[4].opacity, transform: [{ translateY: cardAnims[4].translateY }] }]}>
...
<Animated.View style={[styles.mealsSection, { opacity: cardAnims[5].opacity, transform: [{ translateY: cardAnims[5].translateY }] }]}>
<View style={styles.sectionHeader}>
