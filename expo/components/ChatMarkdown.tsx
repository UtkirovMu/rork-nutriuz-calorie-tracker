...
function renderInlineFormatting(text: string, colors: ThemeColors): React.ReactNode[] {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    parts.push(
      <Text key={`b-${match.index}`} style={{ color: colors.text, fontWeight: '600' as const }}>
        {match[1]}
      </Text>
    );
  ...
  return parts.length > 0 ? parts : [<Text key="full" style={{ color: colors.text }}>{text}</Text>];
}
...
export default React.memo(function ChatMarkdown({ text, colors }: ChatMarkdownProps) {
  ...
  const ds = useMemo(() => StyleSheet.create({
    container: { gap: 2 },
    h1: { fontSize: 17, fontWeight: '700' as const, color: colors.text, marginTop: 8, marginBottom: 4, letterSpacing: -0.4 },
    h2: { fontSize: 16, fontWeight: '600' as const, color: colors.text, marginTop: 6, marginBottom: 3, letterSpacing: -0.3 },
    h3: { fontSize: 15, fontWeight: '600' as const, color: colors.text, marginTop: 5, marginBottom: 2, letterSpacing: -0.2 },
    hr: { height: StyleSheet.hairlineWidth, backgroundColor: colors.textTertiary, marginVertical: 10, opacity: 0.25 },
    bulletRow: { flexDirection: 'row', paddingLeft: 4, marginVertical: 2 },
    bulletDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.primary, marginRight: 10, marginTop: 8 },
    numberedBadge: { minWidth: 20, marginRight: 8, marginTop: 1 },
    numberedText: { fontSize: 14, fontWeight: '600' as const, color: colors.primary, lineHeight: 22 },
    bulletText: { flex: 1, fontSize: 15, lineHeight: 22, color: colors.text, letterSpacing: -0.2 },
    bodyText: { fontSize: 15, lineHeight: 22, color: colors.text, marginVertical: 1, letterSpacing: -0.2 },
    emptyLine: { height: 6 },
    tableContainer: {
      borderRadius: 12,
      overflow: 'hidden',
