...
const defaultAuth: AuthData = {
  isLoggedIn: true, // Bypassed for UI redesign
  loginMethod: 'email',
...
export const [AuthProvider, useAuth] = createContextHook(() => {
  ...
  const sendCodeMutation = useMutation({
    mutationFn: async (params: { method: 'email' | 'phone'; identifier: string }) => {
      console.log('[Auth] Mocking send code to:', params.identifier);
  ...
  const verifyCodeMutation = useMutation({
    mutationFn: async (params: { method: 'email' | 'phone'; identifier: string; code: string }) => {
      console.log('[Auth] Mocking verify code for:', params.identifier);
      // const result = await authApi.verifyCode(params.method, params.identifier, params.code);
      return { token: 'mock-token', isNewUser: false };
    },
  });
  ...
  const logout = useCallback(async () => {
    try {
      ...
      console.log('[Auth] Mocking API logout');
    } catch (e) {
      console.log('[Auth] API logout error (ignored):', e);
    }
    await saveAuthMutation.mutateAsync({ ...defaultAuth, isLoggedIn: false }); // Ensure logout actually clears state
  }, [saveAuthMutation]);
