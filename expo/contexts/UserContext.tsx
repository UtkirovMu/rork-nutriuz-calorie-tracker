...
const defaultProfile: UserProfile = {
  ...
  onboardingComplete: true, // Bypassed for UI redesign
};
...
export const [UserProvider, useUser] = createContextHook(() => {
  ...
  const [healthData, setHealthData] = useState<HealthData>({ steps: 0, distance: 0, activeCalories: 0, isSynced: false });
  ...
  const saveProfileMutation = useMutation({
    mutationFn: async (newProfile: UserProfile) => {
      ...
      try {
        ...
        console.log('[UserContext] Mocking API profile save');
      } catch (e) {
    ...
    mutationFn: async (params: { meals: MealEntry[]; newMeal?: MealEntry }) => {
      ...
      if (params.newMeal) {
        try {
          ...
          console.log('[UserContext] Mocking API meal add');
        } catch (e) {
  ...
  const removeMealMutation = useMutation({
    mutationFn: async (params: { id: string; updatedMeals: MealEntry[] }) => {
      ...
      try {
        ...
        console.log('[UserContext] Mocking API meal remove');
      } catch (e) {
  ...
  const saveWeightMutation = useMutation({
    mutationFn: async (params: { entries: WeightEntry[]; date: string; weight: number }) => {
      ...
      try {
        ...
        console.log('[UserContext] Mocking API weight add');
      } catch (e) {
    ...
    mutationFn: async (params: { entries: UnlockedAchievement[]; newId?: AchievementId }) => {
      ...
      if (params.newId) {
        try {
          ...
          console.log('[UserContext] Mocking API achievement unlock');
        } catch (e) {
    ...
    mutationFn: async (params: { entries: ProgressPhoto[]; newPhoto?: ProgressPhoto; removeId?: string }) => {
      ...
      if (params.newPhoto) {
        try {
          ...
          console.log('[UserContext] Mocking API photo add');
        } catch (e) {
      ...
      if (params.removeId) {
        try {
          ...
          console.log('[UserContext] Mocking API photo remove');
        } catch (e) {
  ...
  const saveStreakMutation = useMutation({
    mutationFn: async (data: StreakData) => {
      ...
      try {
        ...
        console.log('[UserContext] Mocking API streak update');
      } catch (e) {
  ...
  const syncHealthData = useCallback(async () => {
    ...
    if (authorized) {
      const steps = await healthService.getDailySteps();
      const distance = await healthService.getDistance();
      const calories = await healthService.getCaloriesBurned();
      setHealthData({
        steps,
        distance,
  ...
  return useMemo(() => ({
    ...
    healthData,
    syncHealthData,
  }), [profile, updateProfile, dailyTargets, meals, todayMeals, todayTotals, addMeal, removeMeal, weightHistory, addWeightEntry, achievements, unlockAchievement, isAchievementUnlocked, progressPhotos, addProgressPhoto, removeProgressPhoto, streak, getLast7DaysCalories, getLast30DaysCalories, isLoading, healthData, syncHealthData]);
});
